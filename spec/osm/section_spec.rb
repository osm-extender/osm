...
describe "Section" do
  ...
  describe "Using the API" do
    ...
    before :each do
      ...
      $api.stub(:post_query).with('api.php?action=getUserRoles'){ roles }
    end
...
describe "Online Scout Manager API Strangeness" do
  ...
  it "handles a section with no type" do
    ...
    $api.stub(:post_query).with('api.php?action=getUserRoles'){ body }
  ...
  it "handles an empty array representing no notepads" do
    ...
    $api.stub(:post_query).with("api.php?action=getUserRoles"){ body }
    $api.stub(:post_query).with("api.php?action=getNotepads"){ [] }
  ...
  it "Handles user having access to no sections" do
    $api.stub(:post_query).with("api.php?action=getUserRoles"){ [{"isDefault"=>"1"}] }
