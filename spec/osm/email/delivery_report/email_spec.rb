  ...
  describe 'Fetch email from OSM' do
    ...
    describe 'Error getting meta data' do
      ...
      it "Didn't get a Hash" do
        ...
        expect { OSM::Email::DeliveryReport::Email.fetch_from_osm(api: $api, section: 1, email: 2) }.to raise_error OSM::OSMError, 'Unexpected format for response - got a NilClass'
      end
      ...
      it 'Got an error from OSM' do
        ...
        expect { OSM::Email::DeliveryReport::Email.fetch_from_osm(api: $api, section: 1, email: 2) }.to raise_error OSM::OSMError, 'Error message'
      end
    ...
    it 'Error getting body' do
      expect($api).to receive(:post_query).with('ext/settings/emails/?action=getSentEmail&section_id=1&email_id=2&email=&member_id=').and_return('data' => { 'to' => '1 Recipient', 'from' => '"From" <from@example.com>', 'subject' => 'Subject of email', 'sent' => '16/04/2016 13:45' }, 'status' => true, 'error' => nil, 'meta' => [])
      expect($api).to receive(:post_query).with('ext/settings/emails/?action=getSentEmailContent&section_id=1&email_id=2&email=&member_id=').once { raise OSM::OSMError::NotFound, 'Email not found' }
      expect { OSM::Email::DeliveryReport::Email.fetch_from_osm(api: $api, section: 1, email: 2) }.to raise_error OSM::OSMError, 'Email not found'
    end
