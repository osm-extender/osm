module OSM
  class Invoice < OSM::Model
    class Item < OSM::Model
      ...
      validates_numericality_of :record_id, only_integer: true, greater_than: 0, unless: proc { |r| r.record_id.nil? }
      ...
      def create(api)
        ...
        if data.is_a?(Hash) && data['ok'].eql?(true)
          new_item = invoice.get_items(api, no_read_cache: true).sort_by(&:record_id).last
